...
countries_bonds = df_bonds["Economy"].value_counts().reset_index().sort_values(
    by="Economy", 
    ascending=True
)

countries_bonds = countries_bonds[~countries_bonds["Economy"].isin(
    ["CN", "HK"]
)].reset_index(drop=True).drop(
    "count", 
    axis=1
...
## Subset the bond plot


```{python}
df_bonds_bins = df_bonds.groupby(["Year", "country"]).agg(
    total_green = ("total_green", "sum")
).reset_index()




chart_bonds_to2018 = alt.Chart(df_bonds).mark_line().encode(
    alt.X("Year:O"),
    alt.Y("total_green:Q"),
    alt.Color("Economy:N")
).properties(
    ...
    "(datum.Year == 2017)|(datum.Year == 2017)"
)
