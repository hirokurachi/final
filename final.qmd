...
def calc_annual_average(df, outcome):
    ...
    average[["country", "iso"]] = outcome
    average = average[["country", "iso"]].join(
        average.drop(["country", "iso"], axis=1))
    return average


df_world_average = calc_annual_average(df_index, "World average")
...
df_asean3_average = calc_annual_average(
    df_index[df_index["country"] != "World average"], "ASEAN+3 average")
...
df_index["EPI ASEAN+3 average"] = [epi_average(x, "ASEAN+3 average")
...
# Drop China (outlier), Hong Kong (not included in original df_index), and VN*
...
countries_index = df_index[~df_index["country"].isin(["World average", "ASEAN+3 average"])][["country", "iso"]].value_counts().reset_index(
).sort_values(
    by="iso", 
    ascending=True
...
# Drop 2024 data
df_base_long = df_base_long.loc[df_base_long["Year"] != 2024]
...
chart_issuer_type_mix_kor = alt.Chart(df_base_long).transform_filter(
    ...
    alt.Order("bond_label", sort="ascending")
).properties(
    width=500,
    height=500
)
...
# Melt df for easier handling
df_base_long = df_base.melt(
    id_vars=["Country Name", "Country Code", "Year", "EPI", "EPI World average", "EPI ASEAN+3 average",
             "EPI gap from World average", "EPI gap from ASEAN+3 average", "total_green", "total_sustainability"],
    value_vars=["govt_green", "corp_green",
